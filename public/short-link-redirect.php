...
try { short_link_record_open((int)$link['id'], (string)$link['recipient_ref'], short_link_is_bot((string)($_SERVER['HTTP_USER_AGENT'] ?? '')), kssmi_get_trusted_cloudflare_country() ?? ''); } catch (Throwable $e) { error_log('KSSMI short-link event write failed'); }
