...
if ($isAuthenticated && $tab === 'short-links') {
    ...
    try {
        ...
        if ($trackingId !== false && $trackingId !== null) $shortLinkTracking = short_link_tracking((int)$trackingId);
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #shortLinkMap { height:300px; border:1px solid #d9d9d9; border-radius:8px; background:#f7f7f7; }
        ...
        <?php else: ?>
            ...
            <?php endif; ?>
                ...
                <?php if ($tab === 'short-links'): ?>
                    ...
                    <div class="panel" style="margin-top:14px;"><div class="panel-header" style="display:flex;justify-content:space-between;gap:12px;align-items:center;"><span>Track: <?php echo htmlspecialchars($shortLinkTracking['link']['code']); ?></span><a class="btn btn-secondary btn-small" href="?tab=short-links">Back to links</a></div><div class="panel-body">
                        ...
                        <div class="stats" style="margin-bottom:16px;"><div class="stat-card"><div class="stat-label">Opens</div><div class="stat-value"><?php echo number_format((int)$shortLinkTracking['summary']['opens']); ?></div></div><div class="stat-card"><div class="stat-label">Bots filtered</div><div class="stat-value"><?php echo number_format((int)$shortLinkTracking['summary']['bots']); ?></div></div><div class="stat-card"><div class="stat-label">Last opened (Beijing)</div><div class="stat-value" style="font-size:15px;"><?php echo htmlspecialchars(vjt_format_for_admin($shortLinkTracking['summary']['last_opened'] ?? '')); ?></div></div></div>
                        ...
                        <p style="color:#666;font-size:12px;margin:0 0 10px;">All times are Beijing time. Showing the latest <?php echo count($shortLinkTracking['events']); ?> record(s).</p>
                        <div class="table-wrapper table-compact"><table><thead><tr><th>#</th><th>Opened (Beijing)</th><th>Location</th><th>Type</th><th>CRM / recipient reference</th></tr></thead><tbody><?php foreach ($shortLinkTracking['events'] as $index => $event): ?><tr><td class="short-index"><?php echo $index + 1; ?></td><td><?php echo htmlspecialchars(vjt_format_for_admin($event['opened_at'])); ?></td><td><?php echo htmlspecialchars($event['country'] ? vjt_country_name($event['country']) : '-'); ?></td><td><?php echo $event['event_kind'] === 'bot' ? 'Bot filtered' : 'Open'; ?></td><td><?php echo htmlspecialchars($event['recipient_ref_snapshot'] ?: '-'); ?></td></tr><?php endforeach; ?><?php if (!$shortLinkTracking['events']): ?><tr><td colspan="5" style="text-align:center;color:#888;">No opens recorded yet.</td></tr><?php endif; ?></tbody></table></div>
                    </div></div>
                    <?php else: ?>
...
<script>
(function(){var node=document.getElementById('shortLinkMap');if(!node||!window.L||!window.topojson)return;var rows;try{rows=JSON.parse(node.dataset.locations||'[]');}catch(e){return;}var map=L.map(node,{scrollWheelZoom:true,worldCopyJump:true,zoomControl:true,attributionControl:false}).setView([20,10],2);fetch('/vendor/short-link-map/countries-110m.json',{cache:'force-cache'}).then(function(r){return r.json();}).then(function(world){L.geoJSON(topojson.feature(world,world.objects.countries),{style:{color:'#a9a9a9',weight:0.7,fillColor:'#f2f2f2',fillOpacity:1}}).addTo(map);rows.forEach(function(row){var point=({CN:[35,103],US:[39,-98],CA:[56,-106],MX:[23,-102],BR:[-10,-55],GB:[55,-3],FR:[46,2],DE:[51,10],ES:[40,-4],IT:[42,12],RU:[61,105],TR:[39,35],IN:[21,79],JP:[36,138],KR:[36,128],ID:[-2,118],TH:[15,101],VN:[16,108],SG:[1,104],AU:[-25,134],ZA:[-30,25]}[row.country]);if(!point)return;L.circleMarker(point,{radius:Math.min(14,5+Math.sqrt(Number(row.opens)||1)*2),color:'#fff',weight:2,fillColor:'#2f6fed',fillOpacity:.9}).bindTooltip(row.country+' · '+row.opens+' opens').addTo(map);});});})();
</script>
</body>
</html>
