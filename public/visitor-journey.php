...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #shortLinkMap .leaflet-control-zoom { border:0; box-shadow:0 1px 5px rgba(0,0,0,.18); }
        #shortLinkMap .leaflet-control-zoom a { color:#3c4043; border-color:#e2e5e8; background:#fff; }
        #shortLinkMap .leaflet-control-zoom a:is(:hover, :active, :focus-visible, :focus-within) { background:#f6f7f8; }
        ...
        <?php else: ?>
            ...
            <?php endif; ?>
                ...
                <?php if ($tab === 'short-links'): ?>
                    ...
                    <div class="panel" style="margin-top:14px;"><div class="panel-header" style="display:flex;justify-content:space-between;gap:12px;align-items:center;"><span>Track: <?php echo htmlspecialchars($shortLinkTracking['link']['code']); ?></span><span style="display:flex;gap:6px;align-items:center;"><a class="btn btn-secondary btn-small" href="?tab=short-links<?php echo $shortLinkSearch !== '' ? '&amp;sl_search=' . rawurlencode($shortLinkSearch) : ''; ?>">Back to links</a><?php foreach (['previous' => 'Previous', 'next' => 'Next'] as $direction => $label): $neighbor = $shortLinkTrackingNeighbors[$direction]; ?><?php if ($neighbor): ?><a class="btn btn-secondary btn-small" style="min-width:72px;text-align:center;" href="?tab=short-links&amp;sl_tracking=<?php echo (int)$neighbor['id']; ?><?php echo $shortLinkSearch !== '' ? '&amp;sl_search=' . rawurlencode($shortLinkSearch) : ''; ?>"><?php echo $label; ?></a><?php else: ?><span class="btn btn-secondary btn-small" style="min-width:72px;text-align:center;opacity:.45;cursor:default;" aria-disabled="true"><?php echo $label; ?></span><?php endif; ?><?php endforeach; ?></span></div><div class="panel-body">
                        ...
                        <p style="margin:0 0 12px;color:#666;"><strong>Status:</strong> <?php echo htmlspecialchars($shortLinkTracking['link']['status']); ?> &nbsp; <strong>Label / customer:</strong> <button type="button" class="short-target-copy" data-copy="<?php echo htmlspecialchars(trim($shortLinkTracking['link']['label'] . ' ' . $shortLinkTracking['link']['campaign'] . ' ' . $shortLinkTracking['link']['recipient_ref']), ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><?php echo htmlspecialchars(trim($shortLinkTracking['link']['label'] . ' ' . $shortLinkTracking['link']['campaign'] . ' ' . $shortLinkTracking['link']['recipient_ref']) ?: '-'); ?></button></p>
                    ...
                    <div class="panel" style="margin-top:14px;"><div class="panel-header" style="display:flex;justify-content:space-between;align-items:center;gap:12px;"><span>Existing distribution links</span><button id="shortExport" type="button" class="btn btn-secondary btn-small">Export CSV</button></div><div class="panel-body">
                        ...
                        <div class="table-wrapper"><table class="short-link-table"><thead><tr><th><input id="shortSelectAll" type="checkbox" aria-label="Select all current links"></th><th>#</th><th>Short link</th><th>Target</th><th>Label / customer</th><th>Status</th><th>Opens</th><th>Last opened</th><th>Actions</th></tr></thead><tbody><?php foreach ($shortLinkRows as $index => $row): $shortLabel = trim($row['label'] . ' ' . $row['campaign'] . ' ' . $row['recipient_ref']); ?><tr data-short-link="https://kssmi.com/<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>" data-short-target="<?php echo htmlspecialchars($row['target_url'], ENT_QUOTES); ?>" data-short-label="<?php echo htmlspecialchars($shortLabel, ENT_QUOTES); ?>"><td><input class="short-row-select" type="checkbox" aria-label="Select <?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>"></td><td class="short-index"><?php echo $index + 1; ?></td><td><button type="button" class="short-target-copy" data-copy="https://kssmi.com/<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><code>https://kssmi.com/<?php echo htmlspecialchars($row['code']); ?></code></button></td><td class="url-cell short-target"><button type="button" class="short-target-copy" data-copy="<?php echo htmlspecialchars($row['target_url'], ENT_QUOTES); ?>" title="<?php echo htmlspecialchars($row['target_url'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><?php echo htmlspecialchars($row['target_url']); ?></button></td><td><button type="button" class="short-target-copy" data-copy="<?php echo htmlspecialchars($shortLabel, ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><?php echo htmlspecialchars($shortLabel ?: '-'); ?></button></td><td><?php echo htmlspecialchars($row['status']); ?></td><td><?php echo number_format((int)$row['opens']); ?> <span style="color:#888;">(+<?php echo number_format((int)$row['bots']); ?> bots)</span></td><td><?php echo htmlspecialchars(vjt_format_for_admin($row['last_opened'] ?? '')); ?></td><td><div class="short-actions"><button class="btn btn-secondary btn-small" type="button" data-copy="https://kssmi.com/<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)">Copy</button><a class="btn btn-secondary btn-small" href="?tab=short-links&amp;sl_tracking=<?php echo (int)$row['id']; ?><?php echo $shortLinkSearch !== '' ? '&amp;sl_search=' . rawurlencode($shortLinkSearch) : ''; ?>">Track</a><button class="btn btn-danger btn-small" type="button" onclick="shortPermanentlyDelete(<?php echo (int)$row['id']; ?>,'<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>')">Delete</button></div></td></tr><?php endforeach; ?><?php if (!$shortLinkRows): ?><tr><td colspan="9" style="text-align:center;color:#888;">No distribution links found.</td></tr><?php endif; ?></tbody></table></div>
                    </div></div>
                    <?php endif; ?>
