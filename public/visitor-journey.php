...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .short-link-table { min-width: 900px; }
        .short-link-table .short-target { max-width: 170px; }
        .short-link-table .short-actions { display: flex; gap: 6px; }
        .short-link-table .short-actions .btn { width: 78px; padding-left: 4px; padding-right: 4px; }
            ...
            <?php endif; ?>
                ...
                <?php if ($tab === 'short-links'): ?>
                    ...
                    <div class="panel" style="margin-top:14px;"><div class="panel-header" style="display:flex;justify-content:space-between;gap:12px;align-items:center;"><span>Track: <?php echo htmlspecialchars($shortLinkTracking['link']['code']); ?></span><a class="btn btn-secondary btn-small" href="?tab=short-links">Back to links</a></div><div class="panel-body">
                        ...
                        <p style="margin:0 0 12px;color:#666;"><strong>Status:</strong> <?php echo htmlspecialchars($shortLinkTracking['link']['status']); ?> &nbsp; <strong>Label / customer:</strong> <?php echo htmlspecialchars(trim($shortLinkTracking['link']['label'] . ' ' . $shortLinkTracking['link']['campaign'] . ' ' . $shortLinkTracking['link']['recipient_ref']) ?: '-'); ?></p>
                        ...
                        <div class="stats" style="margin-bottom:16px;"><div class="stat-card"><div class="stat-label">Opens</div><div class="stat-value"><?php echo number_format((int)$shortLinkTracking['summary']['opens']); ?></div></div><div class="stat-card"><div class="stat-label">Bots filtered</div><div class="stat-value"><?php echo number_format((int)$shortLinkTracking['summary']['bots']); ?></div></div><div class="stat-card"><div class="stat-label">Last opened</div><div class="stat-value" style="font-size:15px;"><?php echo htmlspecialchars($shortLinkTracking['summary']['last_opened'] ?? '-'); ?></div></div></div>
                        <p style="color:#666;font-size:12px;margin:0 0 10px;">This first release records opening time, bot filtering and the optional CRM reference. It does not collect visitor location. Showing the latest <?php echo count($shortLinkTracking['events']); ?> record(s).</p>
                        <div class="table-wrapper table-compact"><table><thead><tr><th>#</th><th>Opened</th><th>Type</th><th>CRM / recipient reference</th></tr></thead><tbody><?php foreach ($shortLinkTracking['events'] as $index => $event): ?><tr><td class="short-index"><?php echo $index + 1; ?></td><td><?php echo htmlspecialchars($event['opened_at']); ?></td><td><?php echo $event['event_kind'] === 'bot' ? 'Bot filtered' : 'Open'; ?></td><td><?php echo htmlspecialchars($event['recipient_ref_snapshot'] ?: '-'); ?></td></tr><?php endforeach; ?><?php if (!$shortLinkTracking['events']): ?><tr><td colspan="4" style="text-align:center;color:#888;">No opens recorded yet.</td></tr><?php endif; ?></tbody></table></div>
                    </div></div>
                    <?php else: ?>
                    <div class="panel" style="margin-top:14px;"><div class="panel-header">Existing distribution links</div><div class="panel-body">
                        ...
                        <div class="table-wrapper"><table class="short-link-table"><thead><tr><th>#</th><th>Short link</th><th>Target</th><th>Label / customer</th><th>Status</th><th>Opens</th><th>Last opened</th><th>Actions</th></tr></thead><tbody><?php foreach ($shortLinkRows as $index => $row): ?><tr><td class="short-index"><?php echo $index + 1; ?></td><td><code>https://kssmi.com/<?php echo htmlspecialchars($row['code']); ?></code></td><td class="url-cell short-target"><button type="button" class="short-target-copy" data-copy="<?php echo htmlspecialchars($row['target_url'], ENT_QUOTES); ?>" title="<?php echo htmlspecialchars($row['target_url'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><?php echo htmlspecialchars($row['target_url']); ?></button></td><td><?php echo htmlspecialchars(trim($row['label'] . ' ' . $row['campaign'] . ' ' . $row['recipient_ref'])); ?></td><td><?php echo htmlspecialchars($row['status']); ?></td><td><?php echo number_format((int)$row['opens']); ?> <span style="color:#888;">(+<?php echo number_format((int)$row['bots']); ?> bots)</span></td><td><?php echo htmlspecialchars($row['last_opened'] ?? '-'); ?></td><td><div class="short-actions"><button class="btn btn-secondary btn-small" type="button" data-copy="https://kssmi.com/<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)">Copy</button><a class="btn btn-secondary btn-small" href="?tab=short-links&amp;sl_tracking=<?php echo (int)$row['id']; ?>">Track</a><button class="btn btn-danger btn-small" type="button" onclick="shortPermanentlyDelete(<?php echo (int)$row['id']; ?>,'<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>')">Delete</button></div></td></tr><?php endforeach; ?><?php if (!$shortLinkRows): ?><tr><td colspan="8" style="text-align:center;color:#888;">No distribution links found.</td></tr><?php endif; ?></tbody></table></div>
                    </div></div>
                    <?php endif; ?>
