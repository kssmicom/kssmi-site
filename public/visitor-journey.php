...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .short-world-map { position:relative; height:210px; overflow:hidden; border:1px solid #e0d8cc; border-radius:6px; background:linear-gradient(90deg,transparent 49.8%,#e7e1d9 50%,transparent 50.2%),linear-gradient(#f7f5f1 49.8%,#e7e1d9 50%,#f7f5f1 50.2%); }
        .short-world-map::before { content:'W  ←  country-level opening distribution  →  E'; position:absolute; left:10px; bottom:7px; color:#888; font-size:10px; }
        .short-map-dot { position:absolute; width:12px; height:12px; margin:-6px 0 0 -6px; border-radius:50%; background:#8B7355; border:2px solid #fff; box-shadow:0 1px 3px #777; }
        .short-map-dot span { position:absolute; left:8px; top:-8px; white-space:nowrap; font-size:10px; color:#5D4E37; font-weight:600; }
            ...
            <?php endif; ?>
                ...
                <?php if ($tab === 'short-links'): ?>
                    ...
                    <div class="panel" style="margin-top:14px;"><div class="panel-header" style="display:flex;justify-content:space-between;gap:12px;align-items:center;"><span>Track: <?php echo htmlspecialchars($shortLinkTracking['link']['code']); ?></span><a class="btn btn-secondary btn-small" href="?tab=short-links">Back to links</a></div><div class="panel-body">
                        ...
                        <?php if ($shortLinkTracking['locations']): ?><div style="margin:0 0 16px;"><strong style="display:block;margin-bottom:8px;">Opening locations (country-level)</strong><div class="short-world-map"><?php foreach ($shortLinkTracking['locations'] as $location): $point = short_link_country_point($location['country']); if (!$point) continue; $left = (($point[1] + 180) / 360) * 100; $top = ((90 - $point[0]) / 180) * 100; ?><span class="short-map-dot" style="left:<?php echo $left; ?>%;top:<?php echo $top; ?>%;" title="<?php echo htmlspecialchars(vjt_country_name($location['country']) . ': ' . $location['opens']); ?>"><span><?php echo htmlspecialchars($location['country']); ?></span></span><?php endforeach; ?></div><div class="table-wrapper table-compact" style="margin-top:8px;"><table><thead><tr><th>Country</th><th>Opens</th><th>Last opened (Beijing)</th></tr></thead><tbody><?php foreach ($shortLinkTracking['locations'] as $location): ?><tr><td><?php echo htmlspecialchars(vjt_country_name($location['country'])); ?></td><td><?php echo number_format((int)$location['opens']); ?></td><td><?php echo htmlspecialchars(vjt_format_for_admin($location['last_opened'])); ?></td></tr><?php endforeach; ?></tbody></table></div></div><?php else: ?><p style="color:#666;font-size:12px;margin:0 0 10px;">Location data is collected country-by-country for new opens only; existing opens cannot be located retroactively.</p><?php endif; ?>
                    ...
                    <div class="panel" style="margin-top:14px;"><div class="panel-header">Existing distribution links</div><div class="panel-body">
                        ...
                        <div class="table-wrapper"><table class="short-link-table"><thead><tr><th>#</th><th>Short link</th><th>Target</th><th>Label / customer</th><th>Status</th><th>Opens</th><th>Last opened (Beijing)</th><th>Actions</th></tr></thead><tbody><?php foreach ($shortLinkRows as $index => $row): ?><tr><td class="short-index"><?php echo $index + 1; ?></td><td><button type="button" class="short-target-copy" data-copy="https://kssmi.com/<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><code>https://kssmi.com/<?php echo htmlspecialchars($row['code']); ?></code></button></td><td class="url-cell short-target"><button type="button" class="short-target-copy" data-copy="<?php echo htmlspecialchars($row['target_url'], ENT_QUOTES); ?>" title="<?php echo htmlspecialchars($row['target_url'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><?php echo htmlspecialchars($row['target_url']); ?></button></td><td><button type="button" class="short-target-copy" data-copy="<?php echo htmlspecialchars(trim($row['label'] . ' ' . $row['campaign'] . ' ' . $row['recipient_ref']), ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><?php echo htmlspecialchars(trim($row['label'] . ' ' . $row['campaign'] . ' ' . $row['recipient_ref']) ?: '-'); ?></button></td><td><?php echo htmlspecialchars($row['status']); ?></td><td><?php echo number_format((int)$row['opens']); ?> <span style="color:#888;">(+<?php echo number_format((int)$row['bots']); ?> bots)</span></td><td><?php echo htmlspecialchars(vjt_format_for_admin($row['last_opened'] ?? '')); ?></td><td><div class="short-actions"><button class="btn btn-secondary btn-small" type="button" data-copy="https://kssmi.com/<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)">Copy</button><a class="btn btn-secondary btn-small" href="?tab=short-links&amp;sl_tracking=<?php echo (int)$row['id']; ?>">Track</a><button class="btn btn-danger btn-small" type="button" onclick="shortPermanentlyDelete(<?php echo (int)$row['id']; ?>,'<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>')">Delete</button></div></td></tr><?php endforeach; ?><?php if (!$shortLinkRows): ?><tr><td colspan="8" style="text-align:center;color:#888;">No distribution links found.</td></tr><?php endif; ?></tbody></table></div>
                    </div></div>
                    <?php endif; ?>
