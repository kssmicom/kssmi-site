...
$shortLinkRows = [];
$shortLinkTotal = 0;
$shortLinkPage = max(1, (int)($_GET['sl_page'] ?? 1));
...
function short_link_country_point(string $country): ?array {
    static $points = ['CN'=>[35,103],'US'=>[39,-98],'CA'=>[56,-106],'MX'=>[23,-102],'BR'=>[-10,-55],'AR'=>[-34,-64],'CL'=>[-33,-71],'GB'=>[55,-3],'FR'=>[46,2],'DE'=>[51,10],'ES'=>[40,-4],'IT'=>[42,12],'NL'=>[52,5],'SE'=>[62,15],'NO'=>[62,10],'PL'=>[52,20],'RU'=>[61,105],'TR'=>[39,35],'SA'=>[24,45],'AE'=>[24,54],'IN'=>[21,79],'JP'=>[36,138],'KR'=>[36,128],'ID'=>[-2,118],'TH'=>[15,101],'VN'=>[16,108],'MY'=>[4,102],'SG'=>[1,104],'PH'=>[13,122],'AU'=>[-25,134],'NZ'=>[-41,174],'ZA'=>[-30,25],'EG'=>[27,30],'NG'=>[9,8],'KE'=>[1,38]];
    return $points[strtoupper($country)] ?? null;
}
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .short-link-table .short-actions { display: flex; gap: 6px; }
        .short-link-table .short-actions .btn { width: 52px; padding-left: 3px; padding-right: 3px; text-align:center; justify-content:center; }
        .short-links-toolbar { justify-content: flex-end; }
        ...
        .short-links-search-clear:is(:hover,:focus-visible) { color:#333; background:#eee; outline:0; }
        ...
        <?php else: ?>
            ...
            <?php endif; ?>
                ...
                <div class="tabs">
                    ...
                    <a href="?tab=short-links" class="tab <?php echo $tab === 'short-links' ? 'active' : ''; ?>">Links</a>
                    <?php if ($tab === 'journey'): ?>
                ...
                <?php if ($tab === 'short-links'): ?>
                    <div class="panel"><div class="panel-header">Short Links</div><div class="panel-body">
                        <?php if ($shortLinkCapacity && (($shortLinkCapacity['pruned_events'] ?? 0) > 0 || ($shortLinkCapacity['global_total'] ?? 0) > ($shortLinkCapacity['global_count'] ?? 0))): ?><p class="success">Older event details are automatically pruned to protect storage. Lifetime opens remain accurate; recent event history is retained.</p><?php endif; ?>
                        <form id="shortDestinationForm" style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-end;"><div style="flex:1;min-width:280px;"><input id="shortTarget" type="url" required maxlength="4096" aria-label="Destination URL" placeholder="https://video.gumlet.io/..." style="width:100%;padding:8px;border:1px solid #ddd;border-radius:4px;"></div><button class="btn btn-primary" type="submit">Find or register destination</button></form>
                        <div id="shortDestinationResult" style="margin-top:12px;"></div>
                        <form id="shortDistributionForm" hidden style="margin-top:14px;padding-top:14px;border-top:1px solid #eee;display:none;gap:8px;flex-wrap:wrap;align-items:center;"><input id="shortDestinationId" type="hidden"><div><label>Label</label><input id="shortLabel" maxlength="256" style="padding:8px;border:1px solid #ddd;border-radius:4px;"></div><div><label>Campaign</label><input id="shortCampaign" maxlength="256" style="padding:8px;border:1px solid #ddd;border-radius:4px;"></div><div><label>Recipient</label><input id="shortRecipient" maxlength="256" style="padding:8px;border:1px solid #ddd;border-radius:4px;"></div><button class="btn btn-primary" type="submit">Create</button></form>
                    </div></div>
                    <?php if ($shortLinkTracking): ?>
                    <div class="panel" style="margin-top:14px;"><div class="panel-header" style="display:flex;justify-content:space-between;gap:12px;align-items:center;"><span>Track: <?php echo htmlspecialchars($shortLinkTracking['link']['code']); ?></span><span style="display:flex;gap:6px;align-items:center;"><?php foreach (['previous' => 'Previous', 'next' => 'Next'] as $direction => $label): $neighbor = $shortLinkTrackingNeighbors[$direction]; ?><?php if ($neighbor): ?><a class="btn btn-secondary btn-small" style="min-width:72px;text-align:center;" href="?tab=short-links&amp;sl_tracking=<?php echo (int)$neighbor['id']; ?><?php echo $shortLinkSearch !== '' ? '&amp;sl_search=' . rawurlencode($shortLinkSearch) : ''; ?>"><?php echo $label; ?></a><?php else: ?><span class="btn btn-secondary btn-small" style="min-width:72px;text-align:center;opacity:.45;cursor:default;" aria-disabled="true"><?php echo $label; ?></span><?php endif; ?><?php endforeach; ?><a class="btn btn-secondary btn-small" style="min-width:52px;text-align:center;" href="?tab=short-links<?php echo $shortLinkSearch !== '' ? '&amp;sl_search=' . rawurlencode($shortLinkSearch) : ''; ?>">Back</a></span></div><div class="panel-body">
                        <p style="margin:0 0 8px;"><strong>Short link:</strong> <button type="button" class="short-target-copy" data-copy="https://kssmi.com/<?php echo htmlspecialchars($shortLinkTracking['link']['code'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)">https://kssmi.com/<?php echo htmlspecialchars($shortLinkTracking['link']['code']); ?></button></p>
                        <p style="margin:0 0 12px;color:#666;"><strong>Label:</strong> <button type="button" class="short-target-copy" data-copy="<?php echo htmlspecialchars(trim($shortLinkTracking['link']['label'] . ' ' . $shortLinkTracking['link']['campaign'] . ' ' . $shortLinkTracking['link']['recipient_ref']), ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><?php echo htmlspecialchars(trim($shortLinkTracking['link']['label'] . ' ' . $shortLinkTracking['link']['campaign'] . ' ' . $shortLinkTracking['link']['recipient_ref']) ?: '-'); ?></button></p>
                        <p style="margin:0 0 12px;color:#666;word-break:break-all;"><strong>Target:</strong> <button type="button" class="short-target-copy" data-copy="<?php echo htmlspecialchars($shortLinkTracking['link']['target_url'], ENT_QUOTES); ?>" title="<?php echo htmlspecialchars($shortLinkTracking['link']['target_url'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><?php echo htmlspecialchars($shortLinkTracking['link']['target_url']); ?></button></p>
                        <div class="stats" style="margin-bottom:16px;"><div class="stat-card"><div class="stat-label">Opens</div><div class="stat-value"><?php echo number_format((int)$shortLinkTracking['summary']['opens']); ?></div></div><div class="stat-card"><div class="stat-label">Bots filtered</div><div class="stat-value"><?php echo number_format((int)$shortLinkTracking['summary']['bots']); ?></div></div><div class="stat-card"><div class="stat-label">Last opened</div><div class="stat-value" style="font-size:15px;"><?php echo htmlspecialchars(vjt_format_for_admin($shortLinkTracking['summary']['last_opened'] ?? '')); ?></div></div></div>
                        <?php if ($shortLinkTracking['locations']): ?><div style="margin:0 0 16px;"><strong style="display:block;margin-bottom:8px;">Recent opening locations (country-level)</strong><div id="shortLinkMap" data-locations="<?php echo htmlspecialchars(json_encode($shortLinkTracking['locations'], JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT), ENT_QUOTES); ?>"></div><div class="table-wrapper table-compact" style="margin-top:8px;"><table><thead><tr><th>Country</th><th>Opens</th><th>Last opened</th></tr></thead><tbody><?php foreach ($shortLinkTracking['locations'] as $location): ?><tr><td><?php echo htmlspecialchars(vjt_country_name($location['country'])); ?></td><td><?php echo number_format((int)$location['opens']); ?></td><td><?php echo htmlspecialchars(vjt_format_for_admin($location['last_opened'])); ?></td></tr><?php endforeach; ?></tbody></table></div></div><?php else: ?><p style="color:#666;font-size:12px;margin:0 0 10px;">Location data is collected country-by-country for new opens only; existing opens cannot be located retroactively.</p><?php endif; ?>
                        <div class="table-wrapper table-compact"><table><thead><tr><th>#</th><th>Opened</th><th>Location</th><th>Recipient</th><th>Recent opens</th></tr></thead><tbody><?php foreach ($shortLinkTracking['events'] as $index => $event): ?><tr><td class="short-index"><?php echo $index + 1; ?></td><td><?php echo htmlspecialchars(vjt_format_for_admin($event['opened_at'])); ?></td><td><?php echo htmlspecialchars($event['country'] ? vjt_country_name($event['country']) : '-'); ?></td><td><?php echo htmlspecialchars($event['recipient_ref_snapshot'] ?: '-'); ?></td><td><?php echo $event['recipient_opens'] === null ? '-' : number_format((int)$event['recipient_opens']); ?></td></tr><?php endforeach; ?><?php if (!$shortLinkTracking['events']): ?><tr><td colspan="5" style="text-align:center;color:#888;">No opens recorded yet.</td></tr><?php endif; ?></tbody></table></div>
                    </div></div>
                    <?php else: ?>
                    <div class="panel" style="margin-top:14px;"><div class="panel-header short-links-toolbar"><form method="get" class="filters short-links-search"><input type="hidden" name="tab" value="short-links"><span class="short-links-search-input"><input type="search" name="sl_search" value="<?php echo htmlspecialchars($shortLinkSearch); ?>" placeholder="Code, URL, label, campaign, recipient" aria-label="Search distribution links"><?php if ($shortLinkSearch !== ''): ?><button class="short-links-search-clear" type="button" title="Clear search" aria-label="Clear search" onclick="window.location.href='?tab=short-links'">×</button><?php endif; ?></span><button class="btn btn-secondary" type="submit">Search</button></form><button id="shortExport" type="button" class="btn btn-secondary btn-small">Export CSV</button></div><div class="panel-body">
                        <div class="table-wrapper"><table class="short-link-table"><thead><tr><th><input id="shortSelectAll" type="checkbox" aria-label="Select all current links"></th><th>#</th><th>Short link</th><th>Target</th><th>Label</th><th>Campaign</th><th>Opens</th><th>Last</th><th>Actions</th></tr></thead><tbody><?php foreach ($shortLinkRows as $index => $row): $shortLabel = trim($row['label']); $shortCampaign = trim($row['campaign']); ?><tr data-short-link="https://kssmi.com/<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>" data-short-target="<?php echo htmlspecialchars($row['target_url'], ENT_QUOTES); ?>" data-short-label="<?php echo htmlspecialchars($shortLabel, ENT_QUOTES); ?>" data-short-campaign="<?php echo htmlspecialchars($shortCampaign, ENT_QUOTES); ?>"><td><input class="short-row-select" type="checkbox" aria-label="Select <?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>"></td><td class="short-index"><?php echo $shortLinkTotal - (($shortLinkPage - 1) * $shortLinkPerPage) - $index; ?></td><td><button type="button" class="short-target-copy" data-copy="https://kssmi.com/<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><code>https://kssmi.com/<?php echo htmlspecialchars($row['code']); ?></code></button></td><td class="url-cell short-target"><button type="button" class="short-target-copy" data-copy="<?php echo htmlspecialchars($row['target_url'], ENT_QUOTES); ?>" title="<?php echo htmlspecialchars($row['target_url'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><?php echo htmlspecialchars($row['target_url']); ?></button></td><td><button type="button" class="short-target-copy short-plain-copy" data-copy="<?php echo htmlspecialchars($shortLabel, ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><?php echo htmlspecialchars($shortLabel ?: '-'); ?></button></td><td><button type="button" class="short-target-copy short-plain-copy" data-copy="<?php echo htmlspecialchars($shortCampaign, ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)"><?php echo htmlspecialchars($shortCampaign ?: '-'); ?></button></td><td><?php echo number_format((int)$row['opens']); ?> <span style="color:#888;">(+<?php echo number_format((int)$row['bots']); ?> bots)</span></td><td><?php echo htmlspecialchars(vjt_format_for_admin($row['last_opened'] ?? '')); ?></td><td><div class="short-actions"><button class="btn btn-danger btn-small" type="button" onclick="shortPermanentlyDelete(<?php echo (int)$row['id']; ?>,'<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>')">Delete</button><button class="btn btn-secondary btn-small" type="button" data-copy="https://kssmi.com/<?php echo htmlspecialchars($row['code'], ENT_QUOTES); ?>" onclick="shortCopy(this.dataset.copy,this)">Copy</button><a class="btn btn-secondary btn-small" href="?tab=short-links&amp;sl_tracking=<?php echo (int)$row['id']; ?><?php echo $shortLinkSearch !== '' ? '&amp;sl_search=' . rawurlencode($shortLinkSearch) : ''; ?>">Track</a></div></td></tr><?php endforeach; ?><?php if (!$shortLinkRows): ?><tr><td colspan="9" style="text-align:center;color:#888;">No distribution links found.</td></tr><?php endif; ?></tbody></table></div><?php if ($shortLinkPages > 1): ?><nav class="short-link-pagination" aria-label="Short link pages"><?php if ($shortLinkPage > 1): ?><a class="btn btn-secondary btn-small" href="?tab=short-links&amp;sl_page=<?php echo $shortLinkPage - 1; ?><?php echo $shortLinkSearch !== '' ? '&amp;sl_search=' . rawurlencode($shortLinkSearch) : ''; ?>">Previous</a><?php endif; ?><span>Page <?php echo $shortLinkPage; ?> of <?php echo $shortLinkPages; ?></span><?php if ($shortLinkPage < $shortLinkPages): ?><a class="btn btn-secondary btn-small" href="?tab=short-links&amp;sl_page=<?php echo $shortLinkPage + 1; ?><?php echo $shortLinkSearch !== '' ? '&amp;sl_search=' . rawurlencode($shortLinkSearch) : ''; ?>">Next</a><?php endif; ?></nav><?php endif; ?>
                    </div></div>
                    <?php endif; ?>
                                ...
                                <div class="table-wrapper table-wide">
                                    <table>
                                        <thead><tr>
                                            ...
                                            <th>Time</th><th>Channel / Event</th><th>Page / Placement</th>
...
function shortCopy(value,button) { function copied(){var status=document.getElementById('shortCopyStatus');if(status)status.textContent='Copied to clipboard.';if(!button)return;button.classList.add('is-copied');setTimeout(function(){button.classList.remove('is-copied');},900);} function legacy(){var area=document.createElement('textarea');area.value=value;area.setAttribute('readonly','');area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();var ok=false;try{ok=document.execCommand('copy');}catch(e){}document.body.removeChild(area);if(ok)copied();else window.prompt('Copy this link:',value);} if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(value).then(copied).catch(legacy);}else legacy(); }
;(function(){var all=document.getElementById('shortSelectAll'),out=document.getElementById('shortExport'),boxes=[].slice.call(document.querySelectorAll('.short-row-select')),last=-1;function rows(){var chosen=boxes.filter(function(b){return b.checked;});return (chosen.length?chosen:boxes).map(function(b){return b.closest('tr');});}function sync(){if(!all)return;all.checked=boxes.length>0&&boxes.every(function(b){return b.checked;});all.indeterminate=boxes.some(function(b){return b.checked;})&&!all.checked;}if(all)all.addEventListener('change',function(){boxes.forEach(function(b){b.checked=all.checked;});sync();});boxes.forEach(function(box,index){box.addEventListener('click',function(e){if(e.shiftKey&&last>=0){var low=Math.min(last,index),high=Math.max(last,index);for(var i=low;i<=high;i++)boxes[i].checked=box.checked;}last=index;sync();});});if(out)out.addEventListener('click',function(){var csv=['Short link,Target,Label,Campaign'].concat(rows().map(function(row){return [row.dataset.shortLink,row.dataset.shortTarget,row.dataset.shortLabel,row.dataset.shortCampaign].map(function(v){return '"'+String(v||'').replace(/"/g,'""')+'"';}).join(',');})).join('\r\n');var blob=new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='kssmi-short-links.csv';a.click();setTimeout(function(){URL.revokeObjectURL(a.href);},0);});})();
function shortSetStatus(id,status) { var message=status==='archived'?'Archive this link? It will return 404.':'Restore this link?'; if(!confirm(message))return;shortApi({action:'status',id:id,status:status}).then(function(){location.reload();}).catch(function(e){alert(e.message);}); }
...
(function(){
  var destinationForm=document.getElementById('shortDestinationForm'),distributionForm=document.getElementById('shortDistributionForm');
  ...
  destinationForm.addEventListener('submit',function(event){
    ...
    shortApi({action:'destination',target_url:document.getElementById('shortTarget').value}).then(function(response){
      ...
      if(response.status===409&&count>0){
        shortDestinationMessage('Found '+count+' existing distribution link'+(count===1?'':'s')+'. Showing matching links below.');
        var url=new URL(window.location.href);
        url.search='';
        url.searchParams.set('tab','short-links');
        url.searchParams.set('sl_search',destination.target_url);
      ...
      shortDestinationMessage(response.status===409?'This destination is registered, but has no distribution link yet. Create one below.':'Destination registered. Create its first distribution link below.');
    }).catch(function(error){shortDestinationMessage(error.message,true);});
  });
  distributionForm.addEventListener('submit',function(event){
    event.preventDefault();
    shortApi({action:'distribution',destination_id:document.getElementById('shortDestinationId').value,label:document.getElementById('shortLabel').value,campaign:document.getElementById('shortCampaign').value,recipient_ref:document.getElementById('shortRecipient').value}).then(function(response){
      shortDestinationMessage('Created https://kssmi.com/'+response.body.link.code);
      shortCopy('https://kssmi.com/'+response.body.link.code);
