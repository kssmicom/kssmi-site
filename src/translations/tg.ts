export default {
    thankyou: {
      title: "Ташаккур!",
      subtitle: "Дархости шумо қабул шуд — тавассути WhatsApp-и боло фавран нависед.",
      message: "Мо ба таваҷҷӯҳи шумо ба Kssmi миннатдорем. Гурӯҳи мо тафсилоти лоиҳаи шуморо дида мебарояд ва дар давоми 24 соат бо шумо тамос хоҳад гирифт.",
      inquiryId: "ID-и Дархости Шумо",
      nextSteps: "Баъд чӣ мешавад?",
      step1Title: "Баррасӣ",
      step1Desc: "Гурӯҳи мо талаботи шуморо дида мебарояд",
      step2Title: "Тамос",
      step2Desc: "Мо дар давоми 24 соат тамос мегирем",
      step3Title: "Ҳамкорӣ",
      step3Desc: "Мо лоиҳаи шуморо муфассал баррасӣ мекунем",
      continueShopping: "Мурури маҳсулотро идома диҳед",
      backHome: "Бозгашт ба Саҳифаи Асосӣ",
    },
    nav: {
      home: "Асосӣ",
      products: "Маҳсулот",
      collection: "Коллексия",
      feature: "Ҳалли",
      about: "Дар бора",
      contact: "Тамос",
      blog: "Блог",
      luxury: "Боҳашамати Фармоишӣ",
      luxury_sunglasses: "Айнаки офтобии Боҳашамат",
      luxury_glasses: "Айнаки Боҳашамат",
      couture: "Силсилаи Haute Couture",
      couture_sunglasses: "Айнаки офтобии Haute Couture",
      couture_glasses: "Айнаки Haute Couture",
      classics: "Силсилаи Классикӣ",
      classics_sunglasses: "Айнаки офтобии Классикӣ",
      classics_glasses: "Айнаки Классикӣ",
      fashion: "Силсилаи Мода",
      acetate_sun: "Айнаки офтобии Атсетат",
      metal_sun: "Айнаки офтобии Металлӣ",
      metal_glass: "Айнаки Металлӣ",
      titanium_sun: "Айнаки офтобии Титанӣ",
      titanium_glass: "Айнаки Титанӣ",
      rimless: "Силсилаи Бе Чаҳорчӯба",
      rimless_sun: "Айнаки офтобии Бе Чаҳорчӯба",
      rimless_glass: "Айнаки Бе Чаҳорчӯба",
      carbon: "Силсилаи Карбон",
      carbon_sun: "Айнаки офтобии Карбон",
      carbon_glass: "Айнаки Карбон",
      collection_men: "Айнаки мардона",
      collection_women: "Айнаки занона",
      collection_unisex: "Айнаки Унисекс",
      collection_new: "Омадагони нав",
      collection_bestsellers: "Серхаридортарин",
      collection_sustainable: "Маҷмӯаи устувор",
      menu: "Меню",
      skipToContent: "Гузаштан ба мундариҷа",

      ariaSearch: "Ҷустуҷӯ",
      ariaClearSearch: "Тоза кардани ҷустуҷӯ",
      ariaOpenMenu: "Кушодани меню",
      ariaCloseMenu: "Пӯшидани меню",
      ariaToggleSubmenu: "Иваз кардани зерменю",
    },
    footer: {
      tagline: "Айнакҳои Оптикӣ ва Офтобӣ",
      brandName: "Kssmi Eyewear",
      brandDesc: "Истеҳсоли айнаки олӣ — OEM ва ODM аз соли 2003",
      quickLinksTitle: "Истинодҳои Зуд",
      about: "Дар бораи мо",
      manufacturing: "Истеҳсолот",
      collection: "Коллексия",
      support: "Дастгирӣ",
      contact: "Тамос бо мо",
      privacy: "Сиёсати Махфият",
      productsTitle: "Категорияи Маҳсулот",
      luxury: "Боҳашамати Фармоишӣ",
      couture: "Силсилаи Haute Couture",
      classics: "Силсилаи Классикӣ",
      fashion: "Силсилаи Мода",
      carbon: "Силсилаи Карбон",
      factoryTitle: "Корхонаи Мо",
      address: "905, Бинои 1, Парки Технологии Тиббии Baiwangda,\nКӯчаи Yuanshan, Ноҳияи Longgang,\nShenzhen, Guangdong, Чин",
      postcode: "Индекси почта",
      email: "sales@kssmi.com",
      emailLabel: "Почтаи электронӣ: ",
      rights: "Ҳамаи ҳуқуқҳо маҳфузанд."
    },
    cta: {
      quote: "Дархост",
      catalog: "Дидани Каталог"
    },
    search: {
      placeholder: "Ҷустуҷӯи маҳсулот...",
      noResults: "Ягон маҳсулот ёфт нашуд"
    },
    floating: {
      inquiry: "Дархост",
      email: "Фиристодани Имейл",
      whatsapp: "WhatsApp",
      backToTop: "Бозгашт ба Боло"
    },
    product: {
      samplePricing: {
        title: "Нархи Намунаҳо",
        readyStockBadge: "Дар анбор",
        readyStock: "Намунаи тайёр аз анбор",
        readyStockDescription: "Як намунаи тайёри ин модел аз анбор.",
        custom: "Намунаи комилан фармоишӣ",
        customDescription: "Нархи ниҳоӣ аз мавод, ранг, логотип ва мушаххасот вобаста аст.",
        additional: "Намунаи иловагии якхелаи фармоишӣ",
        additionalDescription: "Бо ҳамон тарҳ ва мушаххасот, ҳамроҳ бо намунаи аввал фармоиш дода мешавад.",
        piece: "дона",
        priceContext: "Ҳаққи намуна барои тасдиқи лоиҳаи фармоишии шумо аст, на нархи чакана ё истеҳсоли яклухт.",
        creditNote: "Ҳар ду ҳаққи намуна ҳангоми фармоиши яклухти ҳамон модел пурра ба ҳисоб гирифта мешаванд.",
        creditEmphasis: "пурра ба ҳисоб гирифта мешаванд",
        disclaimer: "Интиқол ва андозҳо алоҳида ҳисоб мешаванд. Нархи истеҳсоли оммавӣ алоҳида пешниҳод мешавад."
      },
        goToPage: 'Ба саҳифа',
        goToButton: 'Рафтан',
      itemNo: "Рақами Мол",
      color: "Ранг",
      serviceMode: "Ҳолати Хизматрасонӣ",
      moq: "Миқдори Ҳадди Ақали Фармоиш (MOQ)",
      backToCatalog: "Бозгашт ба Каталог",
      kssmiCollection: "Kssmi Айнак Фармоишсозӣ Истеҳсолкунанда Коллекция",
      customizationPositioning: "Ин модели намунавӣ барои лоиҳаи бренди хусусии шумост, на маҳсулоти чакана. Аз ин модел оғоз карда, мавод, ранг, линза, логотип ва хусусиятҳои интихобшударо фармоишӣ созед.",
      customizeModel: "Барои бренди худ фармоишӣ созед",
      browseReferenceModels: "Моделҳои бештарро бинед",
      referenceModelStart: "Аз ин модели намунавӣ оғоз кунед.",
      referenceModelStartDetail: "Мавод, ранг, линза, логотип ва хусусиятҳои интихобшударо мутобиқ кунед.",
      ownDesignStart: "Аз идеяи худ таҳия кунед.",
      ownDesignStartDetail: "Нақша, сурат, намуна ё расми техникиро фиристед.",
      technicalData: "Маълумоти Техникӣ",
      specificationProfile: "Хусусиятҳои Техникӣ",
      status: "Ҳолат",
      validated: "Тасдиқшуда",
      docRef: "Истиноди Ҳуҷҷат",
      size: "Андоза",
      materials: "Маводҳо",
      frameMaterial: "Маводи Чаҳорчӯба",
      lensMaterial: "Маводи Линза",
      designStyle: "Услуби Тарроҳӣ",
      nosePads: "Гӯшакҳои Бинӣ",
      hinge: "Ҳалқа",
      electroplating: "Гальваника",
      service: "Хизматрасонӣ",
      carbonFiberFeature: "Хусусиятҳои нахи карбон",
      acetateFeature: "Хусусияти атсетат",
      logo: "Логотип",
      logoOptions: "чоп, лазер, лазер бо равған, штампи гарм, металл, кристалл, стикери барҷаста",
      andCustomization: " ва Фармоишӣ",
      nylonLensCustomization: "Линзаи Нейлон ва Фармоишӣ",
      customizationSuffix: " фармоишӣ",
      pcsPerModel: " ад/модел",
      oem: "Истехсолоти Шартномавӣ",
      odm: "Тарроҳӣ ва Истехсолот",
      systemGenerated: "Аз Ҷониби Система Тавлид Шудааст",
      securedBy: "Бо Назорати Сифати Kssmi Таъмин Карда Шудааст",
      // Product Highlights Section
      highlightsLabel: "Хусусиятҳои Асосӣ",
      highlightsTitle: "Зухумот ва Коркард",
      startJourney: "Сафари Худро Оғоз Кунед",
      partnerTitle: "Шарик барои сохтани Бренди Айнаки Боҳашамати Хусусии Шумо",
      partnerDesc: "Бо истеҳсолкунандае ҳамкорӣ кунед, ки зиёда аз як аср пешвоёни соҳа ба он эътимод доранд. Ҳар як ҷанбаро - маводҳо, шаклҳо, рангҳо ва мушаххасоти техникиро пурра фармоишӣ кунед - маҳорати беназир ва сифати олиро дар миқёс таъмин намоед.",
      benefit1: "Даҳсолаҳои таҷриба дар истеҳсоли айнакҳои боҳашамат.",
      benefit2: "Фармоиши дақиқи мавод, тарроҳӣ ва мушаххасоти техникӣ.",
      benefit3: "Назорати истисноии сифат, ки ороиши бенуқсонро кафолат медиҳад.",
      benefit4: "Истеҳсоли пешрафтаи миқёсшаванда бо дақиқии устувор.",
      benefit5: "Маводҳои муосир, ки ба стандартҳои соҳавӣ мувофиқанд.",
      benefit6: "Интиқоли боэътимод, ки ба ҷадвали истеҳсолии шумо мувофиқ аст.",
      projectFormTitle: "Дар бораи лоиҳаи айнаки худ ба мо нақл кунед",
      projectFormDesc: "Он чиро, ки медонед, мубодила кунед, ҳатто агар мушаххасот ҳанӯз ниҳоӣ набошанд. Мо тафсилотро баррасӣ карда, қадами навбатии мувофиқро барои фармоишӣсозӣ, омода кардани намуна ё нархгузорӣ тавсия медиҳем.",
      nameOrg: "Ном / Ширкат ё бренд",
      officialEmail: "Имейли корӣ",
      projectDetails: "Тафсилоти Лоиҳа",
      submitRequest: "Баррасии лоиҳаи маро гиред",
      secure256: "Ирсоли бехатар · Бе уҳдадории харид",
      ndasAvailable: "NDA дастрас · Ҷавоби аввал дар давоми 24 соат",
      contactDirectly: "Ё мустақиман бо мо тамос гиред:",
      relatedSeries: "Силсилаҳои Алоқаманд",
      viewAllCollections: "Дидани Ҳамаи Коллексияҳо",
      allProducts: "Ҳамаи Маҳсулот",
      productsInCategory: "маҳсулот дар ин категория",
      // Sidebar CTA
      needCustomSolutions: "Ба Айнаки Фармоишӣ Эҳтиёҷ Доред?",
      oemServices: "Мо хидматҳои пураи Истехсолоти Шартномавӣ ва Тарроҳӣ ва Истехсолот-ро барои бренди шумо пешниҳод мекунем.",
      // Certifications
      certificationsTitle: "Сертификатсия ва Мутобиқат",
      certificationsSubtitle: "Стандартҳои Байналмилалии Сифат",
      certificationsActive: "Сертификатҳои фаъол — тасдиқшуда ва мувофиқ",
      // Product listing page
      productAvailable: "маҳсулот дастрас",
      productsAvailable: "маҳсулот дастрас",
      showingResults: "Намоиши {start}-{end} аз {total} маҳсулот",
      noProducts: "Маҳсулот дастрас нест.",
      returnHome: "Ба Саҳифаи Асосӣ Бозгашт",
      loadMore: "Маҳсулоти Бештар Бор Кунед",
      // Search page
      searchTitle: "Ҷустуҷӯи Kssmi",
      searchDesc: "Маҳсулот, мақолаҳо ва бештарро ёбед. Дар боло истилоҳи ҷустуҷӯро ворид кунед.",
      recommendedSearches: "Ҷустуҷӯҳои Тавсияшуда",
      noMatches: "Натиҷа Ёфт Нашуд",
      noMatchesDesc: "Мо предметҳеро, ки ба критерияҳои шумо мувофиқат мекунанд, ёфта натавонистем. Лутфан калимаҳои калидии худро такмил диҳед.",
      backToHome: "Ба Саҳифаи Асосӣ Бозгашт",
      resultsFound: "Натиҷа Ёфт Шуд",
      foundMatches: "Мувофиқат барои ёфт шуд",
      // Product card
      viewDetails: "Маълумоти Бештар",
      customizable: "Фармоишшаванда",
        customizableBadge: "Омода барои бренди хусусӣ",
      previousPage: "Қаблӣ",
      nextPage: "Баъдӣ",
      listingSeoDescription: "Маҳсулоти айнаки премиуми Kssmi-ро ({count} адад) бубинед — истеҳсоли фармоишии OEM/ODM барои брендҳо ва фурӯшандагони ҷаҳонӣ.",
      categorySeoDescription: "Коллексияи {category}-ро кашф кунед — {count} услуб мавҷуд аст. Истеҳсоли айнаки фармоишии OEM/ODM аз Kssmi.",
    },
    layout: {
      description: "Истеҳсоли айнакҳои премиум B2B аз Чин. Чаҳорчӯбаҳои айнаки фармоишии OEM/ODM барои фурӯшандагон ва брендҳои ҷаҳонӣ.",
      schemaDescription: "Kssmi Eyewear як истеҳсолкунандаи касбии айнакҳои B2B мебошад, ки ба чаҳорчӯбаҳои оптикии баландсифати OEM/ODM ва айнакҳои офтобӣ тахассус дорад ва аз соли 2003 мустақиман аз корхона дар Чин фаъолият мекунад.",
    },
    sidebar: {
      // Primary Categories
      sunglasses: "Офтобӣ",
      optical_frames: "Чаҳорчӯбаи Оптикӣ",
      acetate_sunglasses: "Офтобии Атсетатӣ",
      metal_sunglasses: "Офтобии Металлӣ",
      titanium_sunglasses: "Офтобии Титаний",
      carbon_fiber_sunglasses: "Офтобии Нахи Карбон",
      rimless_sunglasses: "Офтобии Бечорчӯба",
      acetate_glasses: "Чаҳорчӯбаи Атсетатӣ",
      metal_glasses: "Чаҳорчӯбаи Металлӣ",
      titanium_glasses: "Чаҳорчӯбаи Титаний",
      carbon_glasses: "Чаҳорчӯбаи Нахи Карбон",
      rimless_glasses: "Чаҳорчӯбаи Бечорчӯба",
      // Series Tags
      fashion: "Айнаки Муд",
      luxury: "Айнаки Лукс",
      classics: "Классикӣ",
      carbon_fiber: "Айнаки Нахи Карбон",
      rimless: "Айнаки Бечорчӯба",
      // Combined Tags
      fashion_acetate_sunglasses: "Офтобии Муд Атсетатӣ",
      fashion_metal_sunglasses: "Офтобии Муд Металлӣ",
      fashion_acetate_glasses: "Оптикии Муд Атсетатӣ",
      fashion_metal_glasses: "Оптикии Муд Металлӣ",
      fashion_acetate_optical_frames: "Чаҳорчӯбаи Оптикии Муд Атсетатӣ",
      fashion_metal_optical_frames: "Чаҳорчӯбаи Оптикии Муд Металлӣ",
      luxury_acetate_sunglasses: "Офтобии Лукс Атсетатӣ",
      luxury_titanium_sunglasses: "Офтобии Лукс Титаний",
      luxury_acetate_glasses: "Оптикии Лукс Атсетатӣ",
      luxury_titanium_glasses: "Оптикии Лукс Титаний",
      luxury_acetate_optical_frames: "Чаҳорчӯбаи Оптикии Лукс Атсетатӣ",
      luxury_titanium_optical_frames: "Чаҳорчӯбаи Оптикии Лукс Титаний",
      classics_acetate_sunglasses: "Офтобии Классикӣ Атсетатӣ",
      classics_metal_sunglasses: "Офтобии Классикӣ Металлӣ",
      classics_acetate_glasses: "Чаҳорчӯбаи Классикӣ Атсетатӣ",
      classics_metal_glasses: "Чаҳорчӯбаи Классикӣ Металлӣ",
      // UI Labels
      tags_filters: "Тегҳо ва Филтрҳо",
      all_products: "Ҳамаи Маҳсулот",
      all_posts: "Ҳамаи Навиштаҳо",
      all_features: "Ҳалли",
      view_all_collections: "Дидани Ҳамаи Маҷмӯаҳо",
      products_in_category: "маҳсулот дар ин категория",
      fashion_group: "Айнаки Муд",
      luxury_group: "Айнаки Лукс",
      carbon_fiber_group: "Айнаки Нахи Карбон",
      rimless_group: "Айнаки Бечорчӯба",
    },
    blog: {
      showingResults: 'Намоиши {start}-{end} аз {total} мақолаҳо',
      emptyState: "Ҳоло ягон мақола нашр нашудааст. Ба наздикӣ тафтиш кунед!",
    },
      pagination: {
      page: "Саҳ. {n}",
    },
      feature: {
      showingResults: "Намоиши {start}-{end} аз {total} хусусият",
      emptyState: "Ҳанӯз хусусиятҳо нест. Ба наздикӣ тафтиш кунед!",
    },
  } as const;
