export default {
    thankyou: {
      title: "Matur Nuwun!",
      subtitle: "Penyelakon Sampeyan Uwis Kita Tampa",
      message: "Matur nuwun dhumateng pitakon sampeyan ing Kssmi Eyewear. Tim kami bakal mriksa detail proyek sampeyan lan mbaleni wektu 24 jam.",
      inquiryId: "ID Panjaluk Sampeyan",
      nextSteps: "Langkah-langkah Lanjut?",
      step1Title: "Akan Disemak",
      step1Desc: "Tim Kami Bakal Mrikso Proyek",
      step2Title: "Hubungi",
      step2Desc: "Kami Bakal Ngetung Kembali Wektu 24 Jam",
      step3Title: "Diskuseni",
      step3Desc: "Akan Bahas Kanggo Detail",
      continueShopping: "Baleni Katon Produk Kami",
      backHome: "Balik Kanggo Halaman Utama",
    },
    nav: {
      home: "Omah",
      products: "Produk",
      collection: "Koleksi",
      feature: "Solusi",
      about: "Babagan",
      contact: "Hubungi",
      blog: "Blog",
      luxury: "Klasik Mewah",
      luxury_sunglasses: "Kacamata Sun Kelas",
      luxury_glasses: "Kacamata Kelas Mewah",
      couture: "Koleksi Haute Couture",
      couture_sunglasses: "Kacamata Sun Haute Couture",
      couture_glasses: "Kacamata Haute Couture",
      classics: "Koleksi Klasik",
      classics_sunglasses: "Kacamata Matahari Klasik",
      classics_glasses: "Kacamata Klasik",
      fashion: "Seri Fashion",
      acetate_sun: "Kaca Mata Acetate",
      metal_sun: "Kaca Mata Logam",
      metal_glass: "Kaca Mata Logam",
      titanium_sun: "Kaca Mata Titanium",
      titanium_glass: "Kaca Mata Titanium",
      rimless: "Koleksi Rimless",
      rimless_sun: "Kacamata Sun Tanpa Rim",
      rimless_glass: "Kacamata Tanpa Rimless",
      carbon: "Koleksi Carbon",
      carbon_sun: "Kacamata Sun Carbon",
      carbon_glass: "Kacamata Karbon",
      collection_men: "Pria",
      collection_women: "Wanita",
      collection_unisex: "Seri Unisex",
      collection_new: "Baru Datang",
      collection_bestsellers: "Kacamata Favorit",
      collection_sustainable: "Sere Sustanabel",
      menu: "Menu",
      skipToContent: "Langsung menyang isi",

      ariaSearch: "Nggoleki",
      ariaClearSearch: "Mbusak nggoleki",
      ariaOpenMenu: "Bukak menu",
      ariaCloseMenu: "Nutup menu",
      ariaToggleSubmenu: "Ganti submenu",
    },
    footer: {
      tagline: "Kacamata Optik & Sun Solutions",
      brandName: "Kssmi Eyewear",
      brandDesc: "Manufaktur Kacamata Premium — OEM & ODM Wiwit 2003",
      quickLinksTitle: "Tautan",
      about: "Babagan Kita",
      manufacturing: "Pabrik",
      collection: "Koleksi",
      support: "Bantuan",
      contact: "Hubungi Kami",
      privacy: "Panduan Privasi",
      productsTitle: "Kategori Produk",
      luxury: "Koleksi Klasik",
      couture: "Koleksi Haute Couture",
      classics: "Koleksi Klasik",
      fashion: "Koleksi Fashion",
      carbon: "Koleksi Karbon",
      factoryTitle: "Pabrik Kita",
      address: "905, Jl Kesehatan Baiwangda \nTaman Teknologi, Jl. Yuanshan, Distrik Longgang\n Shenzen, Guangdong, China",
      postcode: "Kode Pos",
      email: "sales@kssmi.com",
      emailLabel: "Email:",
      rights: "Hak Cipta Dilindhungi."
    },
    cta: {
      quote: "Nyuwun",
      catalog: "Tingali Katalog"
    },
    search: {
      placeholder: "Luru produk...",
      noResults: "Ora Nesuwi Apa"
    },
    floating: {
      inquiry: "Nyuwun",
      email: "Hub Kami Email",
      whatsapp: "Hub Kami WhatsApp",
      backToTop: "Kembali Katon Ndhuwur"
    },
    product: {
      samplePricing: {
        title: "Rega Sampel",
        readyStockBadge: "Stok siap",
        readyStock: "Sampel stok siap",
        readyStockDescription: "Siji sampel stok siap saka model iki.",
        custom: "Sampel kustom",
        customDescription: "Rega pungkasan gumantung bahan, warna, logo lan spesifikasi.",
        additional: "Sampel kustom padha tambahan",
        additionalDescription: "Desain lan spesifikasi padha, dipesen bebarengan karo sampel kustom pisanan.",
        piece: "pcs",
        disclaimer: "Pangiriman lan pajak diitung kapisah. Rega produksi massal ditawakake kapisah."
      },
        goToPage: 'Menyang kaca',
        goToButton: 'Menyang',
      itemNo: "No Barang",
      color: "Warna",
      serviceMode: "Service Mode",
      moq: "Tiyangan Panjaluk",
      backToCatalog: "Menyang Produk",
      kssmiCollection: "Kssmi Kacamata Kustomisasi Pabrik Koleksi",
      technicalData: "Sarat Teknisi",
      specificationProfile: "Spesifikasi Teknis",
      status: "Setatus",
      validated: "Bukti Nyata",
      docRef: "Sarat Referensi",
      size: "Ukur",
      materials: "Material",
      frameMaterial: "Frame Materiale",
      lensMaterial: "Lens Bahan Material",
      designStyle: "Tipe Desain",
      nosePads: "Pad Hidung",
      hinge: "Engsel",
      electroplating: "Pelapisan",
      service: "Bantuan Produksi",
      carbonFiberFeature: "Fitur Serat Karbon",
      acetateFeature: "Fitur Asetat",
      logo: "Logo",
      logoOptions: "cetak, laser, laser karo minyak, stiker ngecap panas, stiker kristal, potongan metalik",
      andCustomization: " & Desain Kustomisasi",
      nylonLensCustomization: "Lensa Nylon & Kustomisasi",
      customizationSuffix: " Pangowahan",
      pcsPerModel: " iku/model",
      oem: "Produksi Kontrak",
      odm: "Desain lan Produksi",
      systemGenerated: "Sistem Terbit",
      securedBy: "Wis Aman Dikontrol Kualitas Kssmi",
      // Product Highlights Section
      highlightsLabel: "Titikan Unggulan",
      highlightsTitle: "Fitur lan Aplikasi",
      startJourney: "Daftar Lan Ngeserake Rencanamu",
      partnerTitle: "Kontak Mitra Pribadi Supaya Kancamu Nguripake Pabrik",
      partnerDesc: "Berkolaborasi kanthi mitra ahli kang bisa dadi pimpinan sakdurunge abad kapungkur. Lakukan perombakan kanthi rinciane wiwit saka warna nganti kacamata sampeyan bakal luwih dipercaya.",
      benefit1: "Latar Belakang Ahli Puluhan taun jroning manufaktur kacamata kemewahan.",
      benefit2: "Kustomisasi kualitas akurasi, wangun presisi.",
      benefit3: "Kalayan Kontrol Sing Canggih Lan Luwih Nyaman.",
      benefit4: "Manufaktur sing diakui.",
      benefit5: "Bahan Kelas Wahid.",
      benefit6: "Jatah produksi miturut waktune.",
      projectFormTitle: "Critakna babagan proyek kacamata sampeyan",
      projectFormDesc: "Kandhakna apa sing wis sampeyan ngerteni, sanajan spesifikasine durung final. Kita bakal mriksa rinciane lan menehi saran langkah sabanjure sing paling cocog kanggo kustomisasi, nggawe sampel, utawa nemtokake rega.",
      nameOrg: "Jeneng / Perusahaan utawa merek",
      officialEmail: "Email bisnis",
      projectDetails: "Rincian proyek",
      submitRequest: "Entuk tinjauan proyekku",
      secure256: "Pangiriman aman · Ora ana kewajiban tuku",
      ndasAvailable: "NDA kasedhiya · Wangsulan awal sajrone 24 jam",
      contactDirectly: "Bakal Dikontak Langsung:",
      relatedSeries: "Seri Produk Kula",
      viewAllCollections: "Kaca Panjaluk Kabeh Jinis",
      allProducts: "Kabeh Produk",
      productsInCategory: "Produk Ing Bab Niki",
      needCustomSolutions: "Ngajengi Panjaluk Liyane?",
      oemServices: "Kami ngladeni Produksi Kontrak lan Desain lan Produksi kanggo merek sampeyan.",
      certificationsTitle: "Lisensi Kualitas",
      certificationsSubtitle: "Kelas Standar Internasional",
      certificationsActive: "Diedarke Lan Panjaluk Manajer",
      productAvailable: "Ngatur Barang",
      productsAvailable: "Barang Sawise",
      showingResults: "Saged Dipriksani {total} , saka kaca {start}-{end}",
      noProducts: "Ora ana produk kasedhiya.",
      returnHome: "Balik menyang Kaca Utama",
      loadMore: "Muat Produk Liyane",
      // Search page
      searchTitle: "Goleki Kssmi",
      searchDesc: "Temokake produk, artikel lan liya-liyane. Lebokake tembung panggolèkan ing dhuwur.",
      recommendedSearches: "Panggolèkan Disaranake",
      noMatches: "Ora Ana Asil",
      noMatchesDesc: "Ora bisa nemokake item sing cocog karo kriteria sampeyan. Mangga afiah tembung kunci sampeyan.",
      backToHome: "Balik menyang Kaca Utama",
      resultsFound: "Asil Ditemokake",
      foundMatches: "Temokake cocog kanggo",
      // Product card
      viewDetails: "Deleng Rincian",
      customizable: "Bisa Dicocokake",
      customizableBadge: "Bisa Dicocokake",
      previousPage: "Sadurunge",
      nextPage: "Sabanjuré",
      listingSeoDescription: "Jelajahi {count} produk kacamata premium saka Kssmi — manufaktur khusus OEM/ODM kanggo merek lan pengecer global.",
      categorySeoDescription: "Jelajahi koleksi {category} — {count} gaya tersedia. Manufaktur kacamata OEM/ODM khusus dening Kssmi.",
    },
    layout: {
      description: "Produsen Pembuatan Kaca Mata Premium Sangking Cina kanggi Reseller ing Global.",
      schemaDescription: "Kssmi Kacamata Pabrikan Produsen khusus sing ngirim Frame Pabrikan Optical Manufaktur wiwit saka taun 2003.",
    },
    sidebar: {
      // Primary Categories
      sunglasses: "Kacamata Ireng",
      optical_frames: "Pigura Optik",
      acetate_sunglasses: "Kacamata Asetat",
      metal_sunglasses: "Kacamata Logam",
      titanium_sunglasses: "Kacamata Titanium",
      carbon_fiber_sunglasses: "Kacamata Serat Karbon",
      rimless_sunglasses: "Kacamata Tanpa Rangka",
      acetate_glasses: "Pigura Asetat",
      metal_glasses: "Pigura Logam",
      titanium_glasses: "Pigura Titanium",
      carbon_glasses: "Pigura Serat Karbon",
      rimless_glasses: "Pigura Tanpa Rangka",
      // Series Tags
      fashion: "Kacamata Fesyen",
      luxury: "Kacamata Mewah",
      classics: "Klasik",
      carbon_fiber: "Kacamata Serat Karbon",
      rimless: "Kacamata Tanpa Rangka",
      // Combined Tags
      fashion_acetate_sunglasses: "Kacamata Fesyen Asetat",
      fashion_metal_sunglasses: "Kacamata Fesyen Logam",
      fashion_acetate_glasses: "Asetat Fesyen Optik",
      fashion_metal_glasses: "Logam Fesyen Optik",
      fashion_acetate_optical_frames: "Pigura Optik Fesyen Asetat",
      fashion_metal_optical_frames: "Pigura Optik Fesyen Logam",
      luxury_acetate_sunglasses: "Kacamata Mewah Asetat",
      luxury_titanium_sunglasses: "Kacamata Mewah Titanium",
      luxury_acetate_glasses: "Asetat Mewah Optik",
      luxury_titanium_glasses: "Titanium Mewah Optik",
      luxury_acetate_optical_frames: "Pigura Optik Mewah Asetat",
      luxury_titanium_optical_frames: "Pigura Optik Mewah Titanium",
      classics_acetate_sunglasses: "Kacamata Klasik Asetat",
      classics_metal_sunglasses: "Kacamata Klasik Logam",
      classics_acetate_glasses: "Pigura Klasik Asetat",
      classics_metal_glasses: "Pigura Klasik Logam",
      // UI Labels
      tags_filters: "Tag lan Filter",
      all_products: "Kabeh Produk",
      all_posts: "Kabeh Tulisan",
      all_features: "Solusi",
      view_all_collections: "Deleng Kabeh Koleksi",
      products_in_category: "Produk Ing Kategori Iki",
      fashion_group: "Kacamata Fesyen",
      luxury_group: "Kacamata Mewah",
      carbon_fiber_group: "Kacamata Serat Karbon",
      rimless_group: "Kacamata Tanpa Rangka",
    },
    blog: {
      showingResults: "Showing {start}-{end} of {total} posts",
      emptyState: "Durung ana artikel. Enggal dipriksa maneh!",
    },
      pagination: {
      page: "Kaca {n}",
    },
      feature: {
      showingResults: "Showing {start}-{end} of {total} features",
      emptyState: "Durung ana fitur. Priksa maneh enggal!",
    },
  } as const;
