---
section: 's02-construction'
heading: 'Solusi Konstruksi Pigura Kacamata'
subheading: 'Intine saka kontrol produksi massal: nyegah titik wuta perakitan lan risiko bali sawise adol, mbantu sampeyan ngetung kanthi akurat biaya kebangkrutan lan entuk pencocokan sing nguntungake banget ing antarane lini produk lan saluran dodolan.'
materials:
  - name: 'Kacamata Pigura Lengkap'
    subtitle: 'Proteksi Lensa Maksimum'
    points:
      - 'Tutup lensa 360° sing aman'
      - 'Nyingkirake pinggiran lensa sing kandel'
      - 'Ndhukung kabeh jinis bahan'
    slug: 'full-rim-eyewear'
    image: '/media/products/kas-010-acetate-sunglasses/oem-odm-customized-acetate-sunglasses-kas-010-1.webp'

  - name: 'Kacamata Pigura Setengah'
    subtitle: 'Ringan lan Ora Ngalangi'
    points:
      - 'Pemasangan gantung senar nilon'
      - 'Ngurangi bobot pigura sakabehe'
      - 'Bidang pandang ngisor sing jelas'
    slug: 'half-rim-eyewear'
    image: '/media/products/kjs-020-titanium-sunglasses/oem-odm-customized-titanium-sunglasses-kjs-020-1.webp'

  - name: 'Kacamata Tanpa Pigura'
    subtitle: 'Minimalis lan Ultra Ringan'
    points:
      - 'Pemasangan lensa struktural langsung'
      - 'Mbutuhake lensa kakuwatan tarik dhuwur'
      - 'Jejak rai sing meh ora krasa'
    slug: 'rimless-eyewear'
    image: '/media/products/krs-205-rimless-sunglasses/oem-odm-customized-rimless-sunglasses-krs-205-1.webp'

  - name: 'Kacamata Klip-Tambah'
    subtitle: 'Solusi Optik Serbaguna'
    points:
      - 'Sambungan magnetik sing mulus'
      - 'Langsung saka optik menyang kacamata ireng'
      - 'Profil ngarep sing didadekake siji kanthi tepat'
    slug: 'clip-on-eyewear'
    image: '/media/products/kts-020-titanium-sunglasses/oem-odm-customized-titanium-sunglasses-kts-020-1.webp'

  - name: 'Kacamata Bisa Dilipat'
    subtitle: 'Portabilitas Ultra Kompak'
    points:
      - 'Mekanisme lipatan multi-engsel'
      - 'Profil panyimpenan ukuran kanthong'
      - 'Kontrol toleransi penutupan sing ketat'
    slug: 'folding-eyewear'
    image: '/media/products/kts-207-titanium-sunglasses/oem-odm-customized-titanium-sunglasses-kts-207-1.webp'

  - name: 'Kacamata Bisa Dibalik'
    subtitle: 'Desain Mekanik Lapis Ganda'
    points:
      - 'Mekanisme engsel sing dipasang ing ndhuwur'
      - 'Adaptasi lingkungan sing cepet'
      - 'Estetika fungsional industri retro'
    slug: 'flip-up-eyewear'
    image: '/media/products/kms-117-metal-sunglasses/oem-odm-customized-metal-sunglasses-kms-117-1.webp'
---
